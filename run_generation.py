...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in bar:
        #source
        source_tokens =[tokenizer.cls_token]+tokenizer.tokenize(example.source)[:args.max_source_length-2]+[tokenizer.sep_token]
...
def main():
    ...
    args.output_dir = 'saved_models/generation_fixed_'+str(args.max_source_length)+'_'+str(args.max_target_length)+'/'
    ...
    def collate_batch(batch):
        ...
        attention_mask = np.zeros((batch_size, max_source_len, max_source_len))
        if not_test:
...
# node_ids, attention_mask            
    ...
    if args.do_train:
        ...
        nb_tr_examples, nb_tr_steps,tr_loss,global_step,best_bleu,best_xmatch,best_codebleu,best_loss = 0, 0,0,0,0,0,0,1e6 
        tr_transl_loss = 0
...
#         tr_ast_sim_ret = {'loss':0, 'match':0, 'total':0}
        ...
        for step in bar:
            ...
            source_ids,attention_mask,target_ids,target_mask,target_dfg,target_ast,target_ast_sim = batch
            
            loss, transl_loss, dfg_ret, ast_path_ret = model(source_ids=source_ids, \
...
#                 tr_ast_sim_ret = {'loss':0, 'match':0, 'total':0}
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        loss, transl_loss, dfg_ret, ast_path_ret = model(source_ids=source_ids, attention_mask=attention_mask, \
...
#                     eval_examples = random.sample(eval_examples,min(500,len(eval_examples)))
                ...
                for batch in tqdm(eval_dataloader):
                    ...
                    source_ids,attention_mask = batch                  
                    with torch.no_grad():
                        module = model.module if hasattr(model, 'module') else model
                        preds = module(source_ids, attention_mask=attention_mask)
...
#                         ref = ref.replace('  NEW_LINE', ' NEW_LINE')
                ...
                if dev_bleu>best_bleu:
                    ...
                    output_model_file = os.path.join(output_dir, "pytorch_model.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
                if dev_acc>best_xmatch:
                    logger.info("  Best xMatch:%s",dev_acc)
                    logger.info("  "+"*"*20)
                    best_xmatch=dev_acc
                    # Save best checkpoint for best bleu
                    output_dir = os.path.join(args.output_dir, 'checkpoint-best-xmatch')
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                    output_model_file = os.path.join(output_dir, "pytorch_model.bin")
    ...
    if args.do_test:
        module = model.module if hasattr(model, 'module') else model
        loadpath = os.path.join(args.output_dir, 'checkpoint-best-bleu/pytorch_model.bin')
        logger.info("Loaded model from: %s",loadpath)
        module.load_state_dict(torch.load(loadpath))
        files=['test']
        for idx,split in enumerate(files):   
            ...
            for batch in pbar:
                ...
                with torch.no_grad():
                    ...
                    top_preds = list(preds.cpu().numpy())
                    pred_ids.extend(top_preds)
                curr_p = [tokenizer.decode(id, skip_special_tokens=True, 
                                                 clean_up_tokenization_spaces=False)
                                          for id in top_preds]
                total += len(source_ids)
                end = start + len(source_ids)
                golds = [eg.target for eg in eval_examples[start:end]]
                match += sum([ref.strip().split()==gold.strip().split() for ref,gold in zip(curr_p,golds)])
                start = end
                pbar.set_description(str(match/total*100))
                p.extend(curr_p)
            model.train()
            accs=[]
            with open(os.path.join(args.output_dir,"test_{}.output".format(str(idx))),'w') as f, open(os.path.join(args.output_dir,"test_{}.gold".format(str(idx))),'w') as f1:
                for ref,gold in zip(p,eval_examples):
#                     ref = ref.replace('  NEW_LINE', ' NEW_LINE')
                    f.write(ref.strip().replace('\n', ' ')+'\n')
                    f1.write(gold.target.strip()+'\n')    
                    accs.append(ref.strip().split()==gold.target.strip().split())
            ...
            dev_codebleu = [round(i*100, 2) for i in \
                            ...
                            os.path.join(args.output_dir, "test_{}.output".format(str(idx))).format(split), 'java', keywords_dir)]
